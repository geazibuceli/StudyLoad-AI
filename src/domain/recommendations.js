...
export function buildRecommendations(tasks, features, risk, referenceDate) {
  ...
  if (features.overdueTaskCount > 0) {
    recommendations.push(recommendation({
      ...
      message: "Choose the overdue task with the highest academic priority and define its next concrete step.",
      action: "Reserve one focused block for the highest-priority overdue task, then review any deadline that is no longer realistic.",
  ...
  if (features.loadRatio > 1) {
    recommendations.push(recommendation({
      ...
      action: "Split large tasks into smaller blocks and move a flexible, lower-priority block outside the busiest days.",
      feature: "loadRatio",
      operator: ">",
      threshold: 1,
  ...
  if (features.deadlineCluster3Days >= 3) {
    recommendations.push(recommendation({
      ...
      action: "Advance one flexible task or complete its first milestone before the cluster begins.",
  ...
  if (features.examWithin14DaysCount >= 2) {
    recommendations.push(recommendation({
      ...
      action: "Reserve repeated review blocks now instead of concentrating all preparation immediately before each exam.",
  ...
  if (features.highPriorityTaskCount >= 3) {
    recommendations.push(recommendation({
      ...
      action: "Select at most two immediate priorities and temporarily rank the others by deadline and remaining effort.",
