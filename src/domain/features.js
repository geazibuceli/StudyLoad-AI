...
function calculateDeadlineCluster(tasks, referenceDate) {
  ...
  for (let start = 0; start < EXAM_WINDOW_DAYS; start += 1) {
    ...
    const count = deadlineOffsets.filter(
...
export function buildDailyLoad(tasks, weeklyAvailableHours, referenceDate) {
  ...
  for (const task of activeTasks) {
    ...
    const lastEligibleDay = Math.min(
      FORECAST_DAYS - 1,
...
export function extractFeatures(tasks, weeklyAvailableHours, referenceDate) {
  ...
  const examsWithin14Days = offsets.filter(
    ({ task, daysUntilDeadline }) => (
      task.type === "exam"
      && daysUntilDeadline >= 0
      && daysUntilDeadline < EXAM_WINDOW_DAYS
    ),
  );
  ...
  const nearestDeadlineDays = offsets.length === 0
    ? 30
    : Math.min(30, Math.max(0, Math.min(
      ...offsets.map(({ daysUntilDeadline }) => daysUntilDeadline),
    )));
  const averageProgress = pendingTasks.length === 0
    ? 100
  ...
  return Object.freeze({
    ...
    remainingHours: round(offsets.reduce(
      (total, item) => total + item.remainingHours,
      0,
    )),
    dueWithin7DaysHours: round(dueWithin7Days.reduce(
      (total, item) => total + item.remainingHours,
      0,
    )),
    ...
    highPriorityTaskCount: pendingTasks.filter(
