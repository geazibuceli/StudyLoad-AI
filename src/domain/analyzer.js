...
import {
  ...
  normalizeSimulationAdjustments,
} from "./validation.js";

export const NON_DIAGNOSTIC_DISCLAIMER = "StudyBalance AI estimates academic workload patterns for organizational support only. It does not diagnose stress, burnout, depression, or any health condition, and it does not replace qualified professional support.";
...
function applyAdjustments(tasks, adjustments) {
  const updatesById = new Map(
    adjustments.taskUpdates.map((task) => [task.id, task]),
  );
...
export function simulateRebalance(payload) {
  const normalized = normalizeScheduleInput(payload);
  const adjustments = normalizeSimulationAdjustments(
    payload.adjustments,
    normalized.tasks,
  );
  ...
  const simulatedWeeklyHours = adjustments.weeklyAvailableHours
    ?? normalized.weeklyAvailableHours;
  ...
  const comparison = Object.freeze({
    ...
    peakDailyHoursDelta: round(
      maximumDailyHours(simulated) - maximumDailyHours(baseline),
      2,
    ),
    loadRatioDelta: round(
      simulated.features.loadRatio - baseline.features.loadRatio,
    ),
